...
class HomeViewController: UIViewController, UIScrollViewDelegate {
    ...
    private func createCoachMarker() {
        ...
        guard let marker = coachMarker,
              let info_view = marker.currentInfoView
        else {
            return
        }
...
// MARK: - ADD MARKS
    ...
    private func addMarks() {
        guard let controls = toolbar.openmenu.superview else {
            return
        }
        ...
        coachMarker?.addMark(title: defaultLocalizer.stringForKey(key: "tarif_set"), info: defaultLocalizer.stringForKey(key: "tarif_set_content"), control: cell.settingsButton)
...
extension HomeViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == BalanceSliderView {
            ...
            cell.balanceValue.text = balance_credit + defaultLocalizer.stringForKey(key: "tjs")
            cell.tariffTitle.text = tarif_name
            cell.paymentDate.text = next_apply_date
            if next_apply_date == defaultLocalizer.stringForKey(key: "Not_paid") {
                cell.paymentDate.textColor = .red
            } else {
                cell.paymentDate.textColor = .white
            }
            // cell.image.image = UIImage(named: mainBannerUrl)

            if mainBannerUrl != "" {
                cell.backgroundImage.af_setImage(withURL: URL(string: mainBannerUrl)!)
            } else {
                cell.backgroundImage.image = UIImage(named: "BalanceBack")
            }

            cell.tariffTitle.frame = CGRect(x: 20, y: 105, width: CGFloat(cell.tariffTitle.text!.count * 10 + 20), height: 20)
            cell.settingsButton.frame.origin.x = CGFloat(cell.tariffTitle.text!.count * 10 + 40)
            ...
            cell.numberTitle.text = "(+992) \(first)-\(second)-\(third)-\(UserDefaults.standard.string(forKey: "mobPhone")!.dropFirst(7))"
...
//
//                             let imageURL = URL(string:     "https://zet-mobile.com/app/Pop.png")! // Замените на фактический URL изображения
//                             let popupViewController = PopupViewController(imageURL: imageURL)
//                            popupViewController.modalPresentationStyle = .overFullScreen
//                             popupViewController.nextButton.addTarget(self, action: #selector(openCompetition), for: .touchUpInside)
//                               present(popupViewController, animated: true, completion: nil)
            ...
            if showPopup == true {
                // "https://zet-mobile.com/app/Pop.png"
                let imageURL = URL(string: popupBanner)! // Замените на фактический URL изображения
                let popupViewController = PopupViewController(imageURL: imageURL)
                popupViewController.modalPresentationStyle = .overFullScreen
                popupViewController.nextButton.addTarget(self, action: #selector(openCompetition), for: .touchUpInside)
                present(popupViewController, animated: true, completion: nil)
            ...
            cell.prereg.frame.origin.x = CGFloat(cell.numberTitle.text!.count * 10 + 15)
            ...
            } else if hot_services_data[indexPath.row][0] == "8" {
                cell.image.image = (UserDefaults.standard.string(forKey: "ThemeAppereance") == "dark" ? UIImage(named: "charkhi_iqbol") : UIImage(named: "charkhi_iqbol"))
            }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == HotServicesView {
            print("hot_service_data: \(hot_services_data)")
            ...
            if hot_services_data[indexPath.row][0] == "2" {
                ...
                self.navigationItem.backBarButtonItem = UIBarButtonItem(title: "", style: .plain, target: nil, action: nil)
                navigationController?.pushViewController(DetalizationViewController(), animated: true)// detalization
            } else if hot_services_data[indexPath.row][0] == "7" {
                self.navigationItem.backBarButtonItem = UIBarButtonItem(title: "", style: .plain, target: nil, action: nil)
