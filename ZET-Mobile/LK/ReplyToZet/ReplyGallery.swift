...
//  ZET-Mobile
//
//  Created by ScrumTJ on 06/10/22.
//
...
extension ImagePicker {
    ...
    private func showAlert(targetName: String, completion: ((Bool) -> Void)?) {
        DispatchQueue.main.async { [weak self] in
            ...
            alertVC.addAction(UIAlertAction(title:  defaultLocalizer.stringForKey(key: "Cancel"), style: .cancel, handler: { _ in completion?(false) }))
...
extension ImagePicker: UIImagePickerControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController,
        ...
        if let imageURL = info[UIImagePickerController.InfoKey.referenceURL] as? URL {
            ...
            if asset != nil {
                name_image = asset?.value(forKey: "filename") as! String
            }
            else {
                name_image = "default"
            }
        ...
        if let image = info[.editedImage] as? UIImage {
            delegate?.imagePicker(self, didSelect: image, name: name_image)
            return
        }
